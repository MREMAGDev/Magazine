...
{
  $ad_query="SELECT account, url, image FROM adverts WHERE id = ".$id;
  $ad_result=$dbca->query($ad_query);
  while(list($account, $url, $image) = $ad_result->fetch_row())
  {
    echo '<tr><td valign="top">';
    $ad_str='<a target="_blank" href="'.$url.'">';
#    $ad_str='<a href="#" onclick="javascript:MyPopUpWin('."'".$url."',900 ,900".');">';
#    $ad_str='<a href="'.$url.'">';
